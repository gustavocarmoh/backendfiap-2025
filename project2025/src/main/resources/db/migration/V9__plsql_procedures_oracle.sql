...
-- Package de procedures NutriXpert
CREATE OR REPLACE PACKAGE pkg_nutrixpert_proc AS
    -- Registra alertas nutricionais automáticos
    PROCEDURE proc_registrar_alerta_nutricional(p_user_id IN VARCHAR2, p_dias IN NUMBER, p_alertas OUT NUMBER);

    -- Gera relatório semanal de consumo
    PROCEDURE proc_gerar_relatorio_consumo(p_user_id IN VARCHAR2, p_relatorio OUT CLOB, p_total_dias OUT NUMBER);

    -- Consulta dinâmica de planos nutricionais
    PROCEDURE proc_consulta_dinamica_planos(p_where IN VARCHAR2, p_result OUT SYS_REFCURSOR);
END pkg_nutrixpert_proc;
/

CREATE OR REPLACE PACKAGE BODY pkg_nutrixpert_proc AS
    -- =====================================
    ...
    -- Procedure com Dynamic SQL para consultas flexíveis
    PROCEDURE proc_consulta_dinamica_planos(p_where IN VARCHAR2, p_result OUT SYS_REFCURSOR) IS
        v_sql VARCHAR2(4000);
    BEGIN
        v_sql := 'SELECT * FROM nutrition_plans WHERE ' || p_where;
        OPEN p_result FOR v_sql;
    END proc_consulta_dinamica_planos;
END pkg_nutrixpert_proc;
/

-- Documentação:
-- Package pkg_nutrixpert_proc centraliza procedures de automação e relatórios.
-- Inclui proc_consulta_dinamica_planos, que usa Dynamic SQL para permitir consultas flexíveis em tempo real,
-- agregando valor ao sistema para dashboards e filtros avançados.
